...
def createPoemCrew()->Crew:
    ...
    goal="""Generate a funny, light heartedpoem about {poem_topic}
          with a sentence count of {sentence_count}""",
    ...
    write_poem: Task = Task(
        description="""Write a poem about {poem_topic}.
        Ensure the poem is engaging and adheres to the specified 
        sentence count of {sentence_count}.""",
        agent=poem_writer,
        expected_output='A beautifully crafted poem about {poem_topic}, with exactly {sentence_count} sentences.',
    )
